...
if (!headers_sent()) {
    ...
    // Retro-term mengatur beberapa widget secara dinamis melalui element.style.
    // Izinkan style attribute saja; inline script tetap wajib memakai nonce.
    $styleAttributePolicy = "; style-src-attr 'unsafe-inline'";
    header("Content-Security-Policy: default-src 'self'; base-uri 'self'; frame-ancestors 'self'; form-action 'self'; object-src 'none'; img-src 'self' data:; style-src 'self' 'nonce-" . CSP_NONCE . "'" . $styleAttributePolicy . "; script-src 'self' 'nonce-" . CSP_NONCE . "'; connect-src 'self'");
